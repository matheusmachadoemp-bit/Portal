            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {anexosAntes.map((a, idx) => (
                <span key={idx} className="inline-flex items-center gap-1.5 bg-nord-panel border border-nord-border rounded-lg px-2 py-1 text-xs text-white">
                  {a.name}
                  <button onClick={() => setAnexosAntes((prev) => prev.filter((_, i) => i !== idx))} className="text-nord-gray hover:text-nord-danger">
                    <X size={12} />
                  </button>
                </span>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {anexosDepois.map((a, idx) => (
                <span key={idx} className="inline-flex items-center gap-1.5 bg-nord-panel border border-nord-border rounded-lg px-2 py-1 text-xs text-white">
                  {a.name}
                  <button onClick={() => setAnexosDepois((prev) => prev.filter((_, i) => i !== idx))} className="text-nord-gray hover:text-nord-danger">
                    <X size={12} />
                  </button>
                </span>
              ))}
