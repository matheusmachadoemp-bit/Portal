...
export function ContasReceberClient({
  ...
  const filtered = useMemo(
    ...
    [receivables, filterStatus, filterVencimento, customFrom, customTo, search]
  );
              ...
              <tr key={p.id} className="border-b border-nord-border/50 hover:bg-white/5">
                ...
                <td className="py-2 pr-4">
                  <div className={`flex items-center gap-2 justify-end ${!canCreate ? "hidden" : ""}`}>
                    {p.status !== "PAGO" && (
                      <button onClick={() => markReceived(p)} title="Marcar como recebido" className="text-nord-gray hover:text-nord-success">
                        <Check size={14} />
                      </button>
                    )}
