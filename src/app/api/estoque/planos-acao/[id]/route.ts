...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.evidenciaUrl && !isValidBlobUrl(body.evidenciaUrl)) {
  ...
  if (!(await hasModulePermission(session.user.id, "estoque", "canEdit"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite editar planos de ação." },
      { status: 403 }
    );
  }
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "estoque", "canDelete"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite excluir planos de ação." },
      { status: 403 }
    );
  }
