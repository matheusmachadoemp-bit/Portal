...
export async function PATCH(req: Request) {
  ...
  // Upsert de configuração por categoria (cria na primeira vez, atualiza depois) — mesmo
  // critério usado em estoque/configuracoes: é uma tela de configuração, não um cadastro
  // recorrente, então trata como canEdit.
  if (!(await hasModulePermission(session.user.id, "ficha-tecnica", "canEdit"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite alterar as configurações de qualidade." },
      { status: 403 }
    );
  }
