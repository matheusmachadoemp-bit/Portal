...
/**
 * Card "Metas de [nome do próximo mês]" da Reunião Cozinha — mesmo mecanismo
 * de `/api/reuniao/gerente/metas-proximo-mes` (ver comentário lá), só
 * trocando `meetingKey` para "COZINHA". Rota fina: toda a lógica fica em
...
export async function GET(req: Request) {
  ...
  const metas = ctx.mode === "single" ? await loadMetasProximoMes(ctx.empresa.id, "COZINHA", periodo) : [];
...
export async function POST(req: Request) {
  ...
  if (!(await hasModulePermission(session.user.id, "reuniao", "canCreate"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite cadastrar metas na Reunião Cozinha." },
      { status: 403 }
    );
  }
