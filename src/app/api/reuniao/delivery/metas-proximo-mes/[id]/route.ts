 ...
 * Reunião Delivery — mesmo mecanismo de
 * `/api/reuniao/gerente/metas-proximo-mes/[id]` (ver comentário lá), só
 * trocando `meetingKey` para "DELIVERY".
 */
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "reuniao", "canEdit"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite editar metas da Reunião Delivery." },
      { status: 403 }
    );
  }
  ...
  const meta = await findMetaProximoMesForMeeting(id, empresa.id, "DELIVERY");
  if (!meta) {
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "reuniao", "canDelete"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite excluir metas da Reunião Delivery." },
      { status: 403 }
    );
  }
