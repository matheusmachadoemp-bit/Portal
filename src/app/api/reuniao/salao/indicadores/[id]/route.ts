...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "reuniao", "canEdit"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite editar indicadores da Reunião Salão." },
      { status: 403 }
    );
  }
  ...
  const indicator = await findReuniaoCustomIndicatorForMeeting(id, empresa.id, "SALAO");
  if (!indicator) {
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "reuniao", "canEdit"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite excluir indicadores da Reunião Salão." },
      { status: 403 }
    );
  }
  ...
  // exclui só esse indicador, sem afetar o resto do fechamento do mês (SalaoMeeting).
