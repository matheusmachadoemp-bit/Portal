...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await hasModulePermission(session.user.id, "tarefas", "canCreate"))) {
    return NextResponse.json(
      { error: "Seu perfil de permissão não permite adicionar itens ao checklist da tarefa." },
      { status: 403 }
    );
  }
  ...
  const maxOrder = await prisma.taskChecklistItem.aggregate({ where: { taskId: id }, _max: { order: true } });
  const item = await prisma.taskChecklistItem.create({
    data: { taskId: id, text: body.text, order: (maxOrder._max.order ?? -1) + 1 },
  });
