...
export async function POST() {
  ...
  const result = await syncEmpresaSaiposSales(empresa, defaultRange());
...
export async function GET(req: Request) {
  ...
  const empresas = await prisma.empresa.findMany({
    where: { active: true, saiposSyncEnabled: true, saiposApiToken: { not: null } },
    select: { id: true, saiposApiToken: true },
  });
